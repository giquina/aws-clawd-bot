...
if ($existingSg) {
    ...
    $sgResult = aws ec2 create-security-group --group-name $sgName --description "Security group for ClawdBot" --region $awsRegion | ConvertFrom-Json
...
Write-Host "Step 3/5: Launching EC2 instance..." -ForegroundColor Cyan
...
$instanceResult = aws ec2 run-instances --image-id ami-0c76bd4bd302b30ec --instance-type t2.micro --key-name $keyName --security-group-ids $sgId --user-data $userDataBase64 --region $awsRegion --tag-specifications $tagSpec | ConvertFrom-Json
...
Write-Host "Step 4/5: Waiting for system initialization (3 minutes)..." -ForegroundColor Cyan
...
try {
    ...
    Write-Host "Upload failed - SSH may not be ready yet" -ForegroundColor Yellow
